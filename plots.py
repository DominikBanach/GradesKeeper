...
#this list stores how many grades are in each category
...
#this function generate and show two plots about how many grades from each category user has
    ...
    plt.yticks(np.arange(0, max([categories[6], categories[7], categories[8]]) + 5, step = 1))
...
def average_grade_history_graph(dates_list, values_list):
#this function is used by another function in api.py to generate plot with given data | 2 arguments, no returns

    win3 = plt.figure(figsize = (11,6), dpi = 100)
    plt.subplots_adjust(top = 0.95, right = 0.97, left = 0.07, bottom = 0.13)

    plt.title("Your average grade - history plot", fontweight = "semibold")
    plt.xlabel("dates when the grade was earned [MM-DD]")
    plt.ylabel("value of the average grade [float type]")

    plt.yticks(np.arange(1, 6, step = 0.25))
    plt.xticks(rotation=75)

    plt.plot(dates_list, values_list, linewidth = 3, color = 'c')
    plt.show()

def generate_single_graphs():
#this function generates plots for every subject | no arguments, no returns
    ...
    for subject in subjects:
        ...
        if max(categories) != 0: plt.ylim(0, max(categories) + 2)
        plt.yticks(np.arange(0, max(categories) + 3, step = 1))

        plt.savefig("img/" + subject + ".jpg")
        plt.clf()